...
class OrganizadorWindow extends SimpleWindow<OrganizadorPartido> {
	...
	def crearPanelIzquierdo(Panel panelPadre) {
		...
		comboCriterios.bindItemsToProperty("criterios").setAdapter(
			new PropertyAdapter(typeof(GeneradorDeEquipos), "nombre"))
		...
		comboOrdenamientos.bindItemsToProperty("ordenamientos").setAdapter(
			new PropertyAdapter(typeof(OrdenadorDeJugadores), "nombre"))
